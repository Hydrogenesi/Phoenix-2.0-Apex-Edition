...
def radiative_gradient(rho, T, r, L, M, P): return -3*total_opacity(rho,T)*rho*L/(64*np.pi*5.67e-8*r**2*T**3)
def adiabatic_gradient(gamma=5/3): return (gamma-1)/gamma
def is_convective(rho, T, r, L, M, P): return radiative_gradient(rho,T,r,L,M,P) > adiabatic_gradient()
