# Operator Hub: Apex Knot `A`

## 1) Core Definition
- **Formal definition**: `Apex Knot` maps state according to its governing pillar invariants.
- **Type signature**: `A: State -> State` (or tuple form for merge/split operators).
...
- **Primary engine**: [Dragon Engine](../../../engines/dragon_engine.md)
- **Algorithm sketch**: normalize input → apply `Apex Knot` transform → validate invariants.
...
- Used in [Ceremony Apex Lock](../../Book09_PhoenixArchive/ceremonies/ceremony_apex_lock.md).
...
[A] --> state' 
```
- **Triadic knot position**: Knot lane in the tri-column map.
...
- Related in-pillar operators: [knot_binding](op_knot_binding.md), [cross_pillar_knot](op_cross_pillar_knot.md), [triadic_closure](op_triadic_closure.md).
