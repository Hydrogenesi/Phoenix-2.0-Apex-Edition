# Operator Hub: Recursive `⊛`

## 1) Core Definition
- **Formal definition**: `Recursive` maps state according to its governing pillar invariants.
- **Type signature**: `⊛: State -> State` (or tuple form for merge/split operators).
- **Axioms**: conservation of essential structure, bounded recursion, and apex-compatibility.
- **Codex examples**: `Book07_TrialOfImmortality` and linked ceremony walkthroughs.

## 2) Engine Implementation
- **Primary engine**: [Phoenix Engine](../../../engines/phoenix_engine.md)
- **Algorithm sketch**: normalize input → apply `Recursive` transform → validate invariants.
...
- Used in [Ceremony Recursion](../../Book09_PhoenixArchive/ceremonies/ceremony_recursion.md).
...
[⊛] --> state' 
```
- **Triadic knot position**: Phoenix lane in the tri-column map.
...
## 6) Codex Navigation
- Defining chapters: `Book07_TrialOfImmortality`.
- Usage chapters: Books 09, 11, and 12 operator applications.
- Related in-pillar operators: [genesis](op_genesis.md), [harmonic](op_harmonic.md), [apex](op_apex.md).
- Cross-pillar operators: [lineage_tracking](op_lineage_tracking.md), [identity_anchoring](op_identity_anchoring.md).
