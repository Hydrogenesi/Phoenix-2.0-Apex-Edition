...
def _cached_total_pressure(rho: float, T: float, mu: float = 0.6) -> float:
    return ideal_gas_pressure(rho, T, mu) + radiation_pressure(T)


def total_pressure(rho, T, mu=0.6, use_cache: bool = False):
    if use_cache and np.isscalar(rho) and np.isscalar(T) and np.isscalar(mu):
        return _cached_total_pressure(float(rho), float(T), float(mu))
    return ideal_gas_pressure(rho, T, mu) + radiation_pressure(T)
